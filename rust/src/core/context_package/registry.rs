...
///
/// Verification operates on the document text, never on re-serialized parsed
/// values: number formatting is not preserved across languages (a writer's
/// `1.0` would re-serialize as `1` in JavaScript and break the hash).
...
fn extract_top_level_value_text<'a>(doc: &'a str, member: &str) -> Option<&'a str> {
